/**
 * キャラクターポーズ生成コンポーネント
 * - 画像をアップロードし、ポーズごとに FAL.ai API でキャラポーズ画像を取得（現在はモック）
 * - 生成画像を ZIP で一括ダウンロード
 *
...
/**
 * 在庫の更新
 * 購入に戻す必要があれば increase, 売り切れに変更するのであれば soldOut を返す
 * 0 は最低値
 *
 * @return JSX.Element
...
const CharacterPoseGenerator = (): JSX.Element => {
    ...
    /** FAL.ai API を呼び出して画像 URL を取得（ここではモック） */
    const callFalAPI = async (prompt: string, _imageBase64: string): Promise<string> => {
        // TODO: 実 API 実装
        /* ここに fetch を置き換え */

        /* モック SVG 返却 */
        return `data:image/svg+xml;base64,${btoa(`
          <svg width="512" height="512" xmlns="http://www.w3.org/2000/svg" style="background: linear-gradient(45deg, #667eea 0%, #764ba2 100%);">
            ...
            <rect x="226" y="280" width="60" height="120" rx="30" fill="white" opacity="0.9"/>
            <rect x="180" y="300" width="40" height="80" rx="20" fill="white" opacity="0.8"/>
            <rect x="292" y="300" width="40" height="80" rx="20" fill="white" opacity="0.8"/>
            <rect x="236" y="420" width="20" height="60" rx="10" fill="white" opacity="0.8"/>
            <rect x="256" y="420" width="20" height="60" rx="10" fill="white" opacity="0.8"/>
            <text x="256" y="500" font-family="Arial" font-size="14" text-anchor="middle" fill="white" opacity="0.9">
    ...
    const generatePoseImages = async (): Promise<void> => {
        ...
        if (!apiKey) {
            /* 開発用警告 */
            console.warn('API キーが設定されていません（モック画像を返します）');
        }
        ...
        try {
            ...
            for (let i = 0; i < posePrompts.length; i++) {
                ...
                try {
                    ...
                    console.error(`"${prompt}" の生成に失敗:`, error);
                    results.push({
                        prompt,
                        index: i,
                        imageUrl:
                            ...
                            btoa(
                                `<svg width="512" height="512" xmlns="http://www.w3.org/2000/svg"><rect width="512" height="512" fill="#ffebee"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="Arial" font-size="16" fill="#c62828">Error: ${prompt}</text></svg>`,
                            ),
                    });
                }
                ...
                await new Promise<number>((r) => window.setTimeout(r, 300));
            }
        } catch (error) {
            console.error('画像生成全体でエラー:', error);
    ...
    const downloadAsZip = async (): Promise<void> => {
        ...
        /* 最小 ZIP 実装（正式には JSZip 推奨） */
        const zipFiles: { name: string; data: Blob }[] = [];
        for (const image of generatedImages) {
            try {
                const resp: Response = await fetch(image.imageUrl);
                const blob: Blob = await resp.blob();
                const safePrompt: string = image.prompt.replace(/[^a-zA-Z0-9]/g, '_');
                zipFiles.push({
                    name: `pose_${image.index + 1}_${safePrompt}.png`,
                    data: blob,
                });
            } catch (err) {
        ...
        const listContent: string =
            'ZIP Archive Content:\n' + zipFiles.map((f): string => `- ${f.name}`).join('\n');
        const zipBlob: Blob = new Blob([listContent], { type: 'text/plain' });

        const url: string = URL.createObjectURL(zipBlob);
